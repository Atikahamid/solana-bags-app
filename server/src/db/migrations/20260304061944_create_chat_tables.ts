...
export async function up(knex: Knex): Promise<void> {
  ...
  await knex.schema.createTable('chat_participants', (table) => {
    ...
    table.string('user_id').notNullable().references('privy_id').inTable('users').onDelete('CASCADE');
  ...
  await knex.schema.createTable('chat_messages', (table) => {
    table.uuid('id').primary().defaultTo(knex.raw('gen_random_uuid()'));
    table.uuid('chat_room_id').notNullable().references('id').inTable('chat_rooms').onDelete('CASCADE');
    table.string('sender_id').notNullable().references('privy_id').inTable('users').onDelete('CASCADE');
