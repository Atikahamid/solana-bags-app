...
export const IPFSAwareImage = ({
  ...
  const handleError = (e: any) => {
    ...
    console.log(
      `Image load error: ${e?.nativeEvent?.error || 'Unknown error'}`,
    );
  ...
  return (
    <View
      style={[{ overflow: 'hidden', backgroundColor: COLORS.background }, style]}>
...
export function getValidImageSource(
  image: string | { uri: string } | number | null | undefined,
) {
  if (!image) return DEFAULT_IMAGES.user;

  if (typeof image === 'object') return image;

  // Handle React Native require() numbers
  ...
  if (v.startsWith('ipfs://')) {
    return { uri: `https://nftstorage.link/ipfs/${v.slice(7)}` };
  }

  const ipfsMatch = v.match(/\/ipfs\/(.+)/);
  if (ipfsMatch) {
    return { uri: `https://nftstorage.link/ipfs/${ipfsMatch[1]}` };
  }

  if (/^(Qm|bafy)/.test(v)) {
    return { uri: `https://nftstorage.link/ipfs/${v}` };
  }
