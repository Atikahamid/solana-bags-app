...
console.log(" +_+_+++++++=============================birdeye api key: ",BIRDEYE_API_KEY);
/**
...
export async function fetchTokenPrice(tokenInfo: TokenInfo | null): Promise<number | null> {
  ...
  console.log("tokenInfor: ", tokenInfo);
  // Return null if token is null
  if (!tokenInfo) {
  ...
  try {
    ...
    try {
      ...
      const jupResponse = await fetch(`https://lite-api.jup.ag/price/v3?ids=${tokenInfo.address}`);
      // console.log("jupiter response : ", jupResponse);
      if (jupResponse.ok) {
        const jupData = await jupResponse.json();
        console.log("jupData: ", jupData);
        const tokenData = jupData?.[tokenInfo.address];
        ...
        console.log(`[TokenService] Jupiter API response not OK for ${tokenInfo.symbol}:`, jupResponse.status);
      }
    } catch (err) {
...
export async function estimateTokenUsdValue(
  ...
  try {
    console.log("----------------------estimated token usd value------------------------");
