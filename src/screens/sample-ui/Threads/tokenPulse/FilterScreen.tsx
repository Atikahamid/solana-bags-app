...
import {getUserFilters, saveUserFilters} from './tokenServicefile';
...
export const FiltersScreen: React.FC = () => {
  ...
  const [top10Min, setTop10Min] = useState('');
  const [top10Max, setTop10Max] = useState('');
  const [devHoldMin, setDevHoldMin] = useState('');
  const [devHoldMax, setDevHoldMax] = useState('');
  const [snipersMin, setSnipersMin] = useState('');
  const [snipersMax, setSnipersMax] = useState('');
  const [insidersMin, setInsidersMin] = useState('');
  const [insidersMax, setInsidersMax] = useState('');
  const [bundlesMin, setBundlesMin] = useState('');
  const [bundlesMax, setBundlesMax] = useState('');
  ...
  const [txnsMin, setTxnsMin] = useState('');
  const [txnsMax, setTxnsMax] = useState('');
  const [buysMin, setBuysMin] = useState('');
  const [buysMax, setBuysMax] = useState('');
  const [sellsMin, setSellsMin] = useState('');
  const [sellsMax, setSellsMax] = useState('');

  const [hasTwitter, setHasTwitter] = useState(false);
  const [hasWebsite, setHasWebsite] = useState(false);
  const [hasTelegram, setHasTelegram] = useState(false);
  const [hasAtLeastOneSocial, setHasAtLeastOneSocial] = useState(false);
  ...
  useEffect(() => {
    ...
    const loadFilters = async () => {
      try {
        ...
        setSelectedProtocols(f.selectedProtocols ?? []);
        setIncludeKeywords(f.includeKeywords ?? '');
        setExcludeKeywords(f.excludeKeywords ?? '');

        setDexPaid(!!f.dexPaid);
        setCaEndsPump(!!f.caEndsPump);

        setCurveMin(f.curve?.min?.toString() ?? '');
        setCurveMax(f.curve?.max?.toString() ?? '');

        setAgeMin(f.age?.min?.toString() ?? '');
        setAgeMax(f.age?.max?.toString() ?? '');

        setTop10Min(f.top10Holders?.min?.toString() ?? '');
        setTop10Max(f.top10Holders?.max?.toString() ?? '');

        setDevHoldMin(f.devHolding?.min?.toString() ?? '');
        setDevHoldMax(f.devHolding?.max?.toString() ?? '');

        setSnipersMin(f.snipers?.min?.toString() ?? '');
        setSnipersMax(f.snipers?.max?.toString() ?? '');

        setInsidersMin(f.insiders?.min?.toString() ?? '');
        setInsidersMax(f.insiders?.max?.toString() ?? '');

        setBundlesMin(f.bundles?.min?.toString() ?? '');
        setBundlesMax(f.bundles?.max?.toString() ?? '');

        setHoldersMin(f.holders?.min?.toString() ?? '');
        setHoldersMax(f.holders?.max?.toString() ?? '');

        setLiquidityMin(f.liquidity?.min?.toString() ?? '');
        setLiquidityMax(f.liquidity?.max?.toString() ?? '');

        setVolumeMin(f.volume?.min?.toString() ?? '');
        setVolumeMax(f.volume?.max?.toString() ?? '');

        setMarketCapMin(f.marketCap?.min?.toString() ?? '');
        setMarketCapMax(f.marketCap?.max?.toString() ?? '');

        setTxnsMin(f.txns?.min?.toString() ?? '');
        setTxnsMax(f.txns?.max?.toString() ?? '');

        setBuysMin(f.numBuys?.min?.toString() ?? '');
        setBuysMax(f.numBuys?.max?.toString() ?? '');

        setSellsMin(f.numSells?.min?.toString() ?? '');
        setSellsMax(f.numSells?.max?.toString() ?? '');

        setHasTwitter(!!f.hasTwitter);
        setHasWebsite(!!f.hasWebsite);
        setHasTelegram(!!f.hasTelegram);
        setHasAtLeastOneSocial(!!f.hasAtLeastOneSocial);
      } catch (err) {
  ...
  const handleApplyFilters = async () => {
    ...
    const filters = {
      ...
      dexPaid,
      caEndsPump,
      curve: {min: Number(curveMin) || 0, max: Number(curveMax) || 0},
      age: {min: Number(ageMin) || 0, max: Number(ageMax) || 0},
      top10Holders: {min: Number(top10Min) || 0, max: Number(top10Max) || 0},
      devHolding: {min: Number(devHoldMin) || 0, max: Number(devHoldMax) || 0},
      snipers: {min: Number(snipersMin) || 0, max: Number(snipersMax) || 0},
      insiders: {min: Number(insidersMin) || 0, max: Number(insidersMax) || 0},
      bundles: {min: Number(bundlesMin) || 0, max: Number(bundlesMax) || 0},
      holders: {min: Number(holdersMin) || 0, max: Number(holdersMax) || 0},
      liquidity: {
        min: Number(liquidityMin) || 0,
        max: Number(liquidityMax) || 0,
      },
      volume: {min: Number(volumeMin) || 0, max: Number(volumeMax) || 0},
      marketCap: {
        min: Number(marketCapMin) || 0,
        max: Number(marketCapMax) || 0,
      },
      txns: {min: Number(txnsMin) || 0, max: Number(txnsMax) || 0},
      numBuys: {min: Number(buysMin) || 0, max: Number(buysMax) || 0},
      numSells: {min: Number(sellsMin) || 0, max: Number(sellsMax) || 0},
      hasTwitter,
      hasWebsite,
      hasTelegram,
      hasAtLeastOneSocial,
    ...
    const savefilters = await saveUserFilters(payload);
    console.log('save filters: ', savefilters);
    // navigation.navigate('TokenLaunchScreen' as never);
    navigation.goBack();
  };
                ...
                <View style={styles.row}>
                  <TouchableOpacity
                    ...
                    <View
                      style={[
                        styles.checkboxBox,
                        dexPaid && styles.checkboxBoxChecked,
                      ]}
                    />
                  ...
                  <TouchableOpacity
                    ...
                    <View
                      style={[
                        styles.checkboxBox,
                        caEndsPump && styles.checkboxBoxChecked,
                      ]}
                    />
          ...
          <View style={styles.section}>
            ...
            {expanded === 'Metrics' && (
              <View style={styles.sectionContent}>
                ...
                {/* Dev Holding % */}
                <Text style={[styles.sectionTitle, {marginTop: 16}]}>Txns</Text>
                <View style={styles.row}>
                  <TextInput
                    style={[styles.input, styles.half]}
                    placeholder="Min"
            ...
            {expanded === 'Socials' && (
              <View style={styles.sectionContent}>
                ...
                <View style={styles.row}>
                  <TouchableOpacity
                    style={styles.checkboxRow}
                    onPress={() => setHasTwitter(v => !v)}>
                    <View
                      style={[
                        styles.checkboxBox,
                        hasTwitter && styles.checkboxBoxChecked,
                      ]}
                    />
                  ...
                  <TouchableOpacity
                    style={styles.checkboxRow}
                    onPress={() => setHasWebsite(v => !v)}>
                    <View
                      style={[
                        styles.checkboxBox,
                        hasWebsite && styles.checkboxBoxChecked,
                      ]}
                    />
                  ...
                  <TouchableOpacity
                    style={styles.checkboxRow}
                    onPress={() => setHasTelegram(v => !v)}>
                    <View
                      style={[
                        styles.checkboxBox,
                        hasTelegram && styles.checkboxBoxChecked,
                      ]}
                    />
                  ...
                  <TouchableOpacity
                    style={styles.checkboxRow}
                    onPress={() => setHasAtLeastOneSocial(v => !v)}>
                    <View
                      style={[
                        styles.checkboxBox,
                        hasAtLeastOneSocial && styles.checkboxBoxChecked,
                      ]}
                    />
...
const styles = StyleSheet.create({
  ...
  checkboxActive: {backgroundColor: COLORS.brandBlue},
