...
const AddCashScreen: React.FC = () => {
  ...
  const {mode = 'add'} = (route.params as {mode?: 'add' | 'withdraw'}) || {};
  ...
  if (paymentMethod === 'Coinbase') {
    navigation.navigate(
      isAdd
        ? ('CoinbaseOnRampScreen' as never)
        : ('CoinbaseOffRampScreen' as never),
      {
        walletAddress,
        amount: numericAmount,
      } as never,
  ...
  Alert.alert(
    isAdd ? 'Deposit via Wallet' : 'Withdraw to Wallet',
