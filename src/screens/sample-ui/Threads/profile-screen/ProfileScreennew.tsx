...
export default function ProfileScreenNew() {
  ...
  const syncBackendBalance = async () => {
    if (!privyId) return;

    try {
      const syncbalanceresponse = await fetch(
        `${SERVER_URL}/api/userRoutess/sync-balance-usd`,
        {
          method: 'POST',
      ...
      console.log('sync balcanse usd: ', syncbalanceresponse);
    } catch (err) {
  ...
  //     fetchWalletAssets();
  //     syncBackendBalance(); // 🔥 cache balance in DB
  //   }
  // }, [walletAddress, privyId]);

  const refreshAll = useCallback(async () => {
    if (!privyId || !walletAddress) return;
  ...
  }, [privyId, walletAddress, fetchProfile, fetchUserPnl, fetchWalletAssets]);
            ...
            <View>
              ...
              <View style={styles.balanceSection}>
                ...
                <Text style={[styles.pnlText, {color: pnlColor}]}>
                  {pnlLoading
                    ? 'Calculating PnL...'
                    : `${Number(pnlPercent).toFixed(2)}% $${Number(
                        pnlUsd,
                      ).toFixed(5)} PNL`}
                </Text>
              </View>
              ...
              <View style={styles.bottomFooterRow}>
                ...
                <View style={styles.followBlock}>
                  <View style={styles.followItem}>
                    <Text style={styles.followCount}>4</Text>
                ...
                <View style={styles.metaBlock}>
                  {/* <View style={styles.metaItem}>
                    ...
                    <Text style={styles.metaText}>No hold time</Text>
                  </View> */}

                  <View style={styles.metaItem}>
                    <Icons.Swapsettingsicon
                      width={15}
                      height={15}
                      // color="#00FF77"
                    />
                  ...
                  <View style={styles.metaItem}>
                    <Icons.CalenderIcon
                      ...
                      // color="#00FF77"
                    />
                    <Text style={styles.metaText}>Joined Nov 2025</Text>
                  </View>
                </View>
              </View>
            </View>
